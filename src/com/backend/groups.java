	   ...
	   //-------Variables
	    ...
	    for (int i=start; i<mycards.size();i++) { 
	    	...
	    	for (int j=start+1;j<mycards.size();j++) {
	    		//PENSAR COMO METER LOS JOKERS SIN JODER LA SERIE, COMO ESTÁ AHORA INTERRUMPIRÍA LA SERIE
...
public void lets_play_a(ArrayList<Piece>chips) {
	...
	ArrayList<Piece> all_strats= new ArrayList<>();
	for (int i=0; i<chips.size();i++ ) {
		
		ArrayList<Piece> chips_left = chips;
		ArrayList<Piece> strat=new ArrayList<>();
		if (check_groups(chips))
