...
/**
 * Class to handle the mcts tree and generate new policy vectors.
 * 
...
public class MCTS {
	...
	private Map<String, Double[]> Ps = new HashMap<String, Double[]>(); // stores initial policy (returned by neural
																		// net)
	 ...
	 * 
	 * @param canonicalBoard: initial state
	 * @return probs: a policy vector where the probability of the ith action is
	 ...
	 * 
	 * @param canonicalBoard: initial state
	 * @param temp:           indicates when to pick random actions with probability
	 *                        distribution
	 * @return probs: a policy vector where the probability of the ith action is
	 *               proportional to Nsa[(s,a)]**(1./temp)
	 */
	 ...
	 * maximum upper confidence bound as in the paper. Once a leaf node is found,
	 * the neural network is called to return an initial policy P and a value v for
	 * the state. This value is propagated up the search path. In case the leaf node
	 * is a terminal state, the outcome is propagated up the search path. The values
	 * of Ns, Nsa, Qsa are updated. NOTE: the return values are the negative of the
	 * value of the current state. This is done since v is in [-1,1] and if v is the
	 * value of a state for the current player, then its value is -v for the other
	 * player.of MCTS starting from canonicalBoard.
	 * 
	 * @param canonicalBoard: initial state
	 * @return v: value of the current state, -1 if lost, 1 if win
	 */
		...
		for (int a = 0; a < this.game.getActionSize(); a++) {
			if (valids[a] != 0) {
				...
				if (this.Qsa.containsKey(s_a)) {
					u = this.Qsa.get(s_a)
							+ this.args.cpuct * this.Ps.get(s)[a] * Math.sqrt(this.Ns.get(s)) / (1 + this.Nsa.get(s_a));
				} else {
