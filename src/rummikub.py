...
def lets_play_a(chips):
        all_strats = []
        for num in range(len(chips)):
                chips_left = chips
                out_tup = ()
                strat = []
                if check_groups(chips)[0]: 
                        strat.append(check_groups(chips)[0])
                        chips_left = check_groups(chips)[1]
                points = 0
                if check_series(chips_left, num)[0]:
                        strat.append(check_series(chips_left, num)[0])
                        chips_left = check_series(chips_left, num)[1]
...
print(lets_play_a(chips)[0])
print(lets_play_b(chips)[0])
print(lets_play_c(chips)[0])
